...
def load_scorecards(
    ...
    # For speed records, prefer real speed tests over throughput-as-speed
    ...
    for r in records:
        ...
        if key not in latest:
            ...
            # Prefer speed records with actual TTFT data (real benchmarks)
            # over throughput tests tagged as speed (ttft=0, has concurrency)
            existing = latest[key]
            existing_is_throughput = existing.extra.get("test_type") == "throughput"
            new_is_throughput = r.extra.get("test_type") == "throughput"
            if existing_is_throughput and not new_is_throughput:
                latest[key] = r
    ...
    # Backfill hardware if all cards have empty hardware
    hw_fallback = detect_hardware_string()
    cards = list(by_model.values())
    for card in cards:
        if not card.hardware:
            card.hardware = hw_fallback
...
<div class="footer">
  ...
  <span>Hover heatmap cells for details. Score colors: <span class="score-high">≥80</span> <span class="score-mid">≥50</span> <span class="score-low">&lt;50</span></span>
</div>
    ...
    for c in cards_with_quality:
        ...
        for task_name in all_tasks:
            ...
            if t is None:
                ...
                tps = t.get("tps", 0)
                ttft = t.get("ttft_ms", 0)
                tip = f'{task_name}: '
                if runnable == 1.0:
                    cls = "cell-pass"
                    tip += "runnable + correct"
                elif contains >= 0.8:
                    cls = "cell-pass"
                    tip += f"contains {contains:.0%}"
                elif contains >= 0.5:
                    cls = "cell-partial"
                    tip += f"partial {contains:.0%}"
                else:
                    cls = "cell-fail"
                    tip += "fail"
                if tps > 0:
                    tip += f" | {tps:.1f} tok/s"
                if ttft > 0:
                    tip += f" | TTFT {ttft:.0f}ms"
                cells += f'<td class="{cls}" title="{html.escape(tip)}"></td>'
        model_safe = html.escape(c.model[:30])
        pr = c.quality_data.get("pass_rate", 0)
        pr_cls = "score-high" if pr >= 0.8 else "score-mid" if pr >= 0.5 else "score-low"
        heatmap_rows += f'<tr><td class="model-name">{model_safe} <span class="{pr_cls}">{pr:.0%}</span></td>{cells}</tr>\n'

    # Footer: pass rate per task
    footer_cells = ""
    for task_name in all_tasks:
        total = 0
        passed = 0
        for c in cards_with_quality:
            ...
            if t is not None:
                total += 1
                if t.get("runnable", 0) == 1.0 or t.get("contains", 0) >= 0.8:
                    passed += 1
        pct = (passed / total * 100) if total else 0
        pct_cls = "footer-high" if pct >= 80 else "footer-mid" if pct >= 50 else "footer-low"
        footer_cells += f'<td class="{pct_cls}">{pct:.0f}%</td>'
    footer_row = f'<tr class="pass-rate-footer"><td class="model-name">pass rate</td>{footer_cells}</tr>'
...
<div class="legend" style="margin-bottom:0.5rem">
  ...
  <span class="legend-item"><span class="legend-swatch" style="background:var(--orange)"></span> Partial</span>
    ...
    for c in cards_with_sql:
        ...
        for q in all_questions:
            ...
            if result is None:
                ...
                tip = f'Q{qid}: {status}'
                if retries > 0:
                    tip += f' ({retries} retries)'
                if tps:
                    tip += f' | {tps:.1f} tok/s'
                cells += f'<td class="cell-{status}" title="{html.escape(tip)}"></td>'
        model_safe = html.escape(c.model[:30])
        sql_pr = c.sql_data.get("pass_rate", 0)
        pr_cls = "score-high" if sql_pr >= 0.8 else "score-mid" if sql_pr >= 0.5 else "score-low"
        p = c.sql_data.get("total_pass", 0)
        t = c.sql_data.get("total_questions", 0)
        heatmap_rows += f'<tr><td class="model-name">{model_safe} <span class="{pr_cls}">{p}/{t}</span></td>{cells}</tr>\n'

    # Footer: pass rate per question
    footer_cells = ""
    for q in all_questions:
        qid = q.get("id")
        total = 0
        passed = 0
        for c in cards_with_sql:
            q_map = {qq.get("id"): qq for qq in c.sql_data.get("questions", [])}
            result = q_map.get(qid)
...
def _unified_css() -> str:
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .heatmap-table td:not(.model-name) {
    ...
    padding: 2px; cursor: default; border: 1px solid var(--bg);
  }
  .cell-pass { background: var(--green); }
  .cell-fail { background: var(--red); }
  .cell-error { background: var(--orange); }
  .cell-partial { background: var(--orange); opacity: 0.7; }
  ...
  .pass-rate-footer td {
    ...
    width: auto; min-width: auto; max-width: none; height: auto;
  }
  ...
  /* Legend */
  .legend { display: flex; gap: 1.2rem; font-size: 0.78rem; color: var(--text-dim); }
